...
export class WorkSessionGuard implements CanActivate{
  ...
  async canActivate(context:ExecutionContext){
    ...
    const branchAccountId=request.branchDevice?.registeredById??request.authUser?.id;
    ...
    const session=await this.prisma.workSession.findFirst({
      where:{id:workSessionId,authenticatedBy:branchAccountId,branchId:request.branchDevice?.branchId,status:'ACTIVE',endedAt:null}
    });
