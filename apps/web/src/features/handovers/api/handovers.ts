...
export type HandoverDetail=HandoverSummary&{notes?:string;createdAt?:string;submittedAt?:string|null;confirmedAt?:string|null;lockedAt?:string|null;participants?:HandoverParticipant[];checklistResults?:InventoryCheck[];items:Array<{id:string;title:string;details:string;category:string;priority:string;roomNumber?:string|null}>};
...
async function getHandover(id:string):Promise<HandoverDetail>{
  const{data,error}=await createClient().from('handovers').select(`
    id,code,status,branch_id,notes,created_at,submitted_at,confirmed_at,locked_at,
    participants:handover_participants(
  ...
  return{id:row.id,code:row.code,status:row.status,branchId:row.branch_id,giver:{id:giver?.id??'',name:giver?.fullName??'Người giao'},receiver:{id:receiver?.id??'',name:receiver?.fullName??'Người nhận'},createdAt:row.created_at,...(row.submitted_at?{submittedAt:row.submitted_at}:{}),...(row.notes?{notes:row.notes}:{}),confirmedAt:row.confirmed_at,lockedAt:row.locked_at,participants,checklistResults:row.checklist_results.map(item=>({itemCode:item.item_code,isCompleted:item.is_completed,receiverCheckedAt:item.receiver_checked_at})),items:row.items.map(item=>({id:item.id,title:item.title,details:item.details,category:item.category,priority:item.priority,roomNumber:item.room_number}))};
}
...
export const handoverApi={
  ...
  accountingSign:(id:string,signatureText:string)=>apiRequest<unknown>(`/handovers/${id}/accounting-sign`,{method:'POST',body:JSON.stringify({signatureText})}),
