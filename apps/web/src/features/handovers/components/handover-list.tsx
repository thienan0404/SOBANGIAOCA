...
export function HandoverList(){
  ...
  if(error)return <div role="alert" className="empty error-state"><div className="empty-icon">!</div><strong>Chưa thể kết nối dữ liệu</strong><p>Vui lòng thử tải lại sau ít phút.</p><button onClick={()=>location.reload()}>Thử lại</button></div>;
  ...
  return <><div className="list-status"><span><i/> Đang đồng bộ</span><b>{data.length} phiếu</b></div><div className="card-list">{data.map(x=><a className="handover-card amber" href={`/handovers/${x.id}`} key={x.id}><div className="card-icon">⇄</div><div className="card-body"><div className="card-title"><h3>{x.code}</h3><span>{labels[x.status]}</span></div><p className="detail">{x.giver?.name??'Người giao'} → {x.receiver?.name??'Người nhận'}</p></div><span className="card-chevron">›</span></a>)}</div></>;
}