...
export function CreateHandoverForm(){
  ...
  return <div className="handover-form-page">
    ...
    <section className="handover-meta-card" aria-label="Thông tin ca làm việc">
      <div className="handover-meta-item branch-meta"><span className="meta-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M4 10.5 12 4l8 6.5V20H4z"/><path d="M9 20v-6h6v6"/></svg></span><div className="meta-copy"><small>CHI NHÁNH</small><strong>{branchName}</strong><em className="meta-state"><i/>Điểm làm việc hiện tại</em></div></div>
      <div className="handover-meta-item shift-meta"><span className="meta-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="8"/><path d="M12 8v5l3 2"/></svg></span><div className="meta-copy"><small>CA LÀM VIỆC</small><strong>{loading?'Đang đồng bộ...':shift?`${new Date(shift.startsAt).toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit'})} – ${new Date(shift.endsAt).toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit'})}`:'Chưa có ca phù hợp'}</strong><em className={`meta-state ${shift?'active':''}`}><i/>{loading?'Đang tải dữ liệu':shift?'Ca đang hoạt động':'Chưa xác định ca'}</em></div></div>
    </section>
